...
class EGAT(MessagePassing):
    ...
    def __init__(self,
                 node_in_channels,  # node_in_dim
                 node_out_channels,  # node_out_dim
                 edge_in_channels,  # number of edge features. here, that's edge types
                 edge_out_channels,
                 heads=2,
                 concat=False,
                 negative_slope=0.2,
        ...
        self.em_src_lin = Linear(node_out_channels, heads * node_out_channels, bias=False)
        self.em_dst_lin = Linear(node_out_channels, heads * node_out_channels, bias=False)
    ...
    def forward(self, x, edge_index, edge_attr):
        ...
        # Edge updates
        em_src_x = self.em_src_lin(node_out)
        em_dst_x = self.em_dst_lin(node_out)
    ...
    def edge_update(self, x_i, x_j, edge_attr, index, ptr, size_i):
        ...
        attended_edge_attr = attended_edge_attr.sum(dim=1)  # sum over heads

        # Combine node and edge information
        x_i_mean = x_i.sum(dim=1)
        x_j_mean = x_j.sum(dim=1)
        edge_attr_mean = edge_attr.sum(dim=1)
...
class EGATMethod(MessagePassingGNN):
    def __init__(self,
        ...
        for _ in range(self.propagation_steps):
            self.middle.append(EGAT(node_in_channels=self.node_hidden_size,
                                    node_out_channels=self.node_hidden_size,
                                    edge_in_channels=2,
                                    edge_out_channels=2,
                                    ).to(device))
    ...
    def forward(self, data: Data):
        ...
        out = {'x': x, 'edge_attr': edge_type, 'edge_index': edge_idx}
